...
print("frequency feature",len(frequency_features))
...
# -------------------------------------------------------------
# Perform foward feature selection using simple decision tree
# --------------------------------------------------------------
...
ordered_scores=[0.8827990348155809,
 0.9706997587038952,
 0.9917269906928645,
 0.9972423302309549,
 0.9982764563943468,
...
score_df.sort_values(by="accuracy",ascending=False)
score_df.to_pickle("../../data/interim/score_df.pkl")
...
participant_df = df.drop(["set", "condition"], axis=1)
...
# Remove the "participant" column from X_train and X_test
...
accuracy=accuracy_score(y_test,class_test_y)
print(f"accuracy of RF on participent : {accuracy}")
classes=class_test_prob_y.columns
cm=confusion_matrix(y_test,class_test_y,labels=classes)
# create confusion matrix for cm
plt.figure(figsize=(10, 10))
...
# --------------------------------------------------------------
# Use complex model for results check
# --------------------------------------------------------------
(
class_train_y,
class_test_y,
...
accuracy=accuracy_score(y_test,class_test_y)
print(f"accuracy of NN on participent : {accuracy}")
classes=class_test_prob_y.columns
cm=confusion_matrix(y_test,class_test_y,labels=classes)
# create confusion matrix for cm
plt.figure(figsize=(10, 10))
